...
/* $Id: tab.c,v 1.55 2004/05/25 04:13:40 jonas Exp $ */
...
void
open_url_in_new_tab(struct session *ses, struct uri *uri, int in_background)
{
	struct window *tab;
...
open_current_link_in_new_tab(struct session *ses, int in_background)
{
	...
	open_url_in_new_tab(ses, uri, in_background);
	if (uri) done_uri(uri);
}
