...
/* $Id: mime.c,v 1.13 2003/01/02 05:37:43 pasky Exp $ */
...
menu_del_ext(struct terminal *term, void *fcp, void *xxx2)
{
	...
	msg_box(term, getml(str, translated, NULL),
		_(T_DELETE_EXTENSION), AL_CENTER | AL_EXTD_TEXT,
		_(T_DELETE_EXTENSION), " ", str, "?", NULL,
		translated, 2,
		_(T_YES), really_del_ext, B_ENTER,
		_(T_NO), NULL, B_ESC);
...
static struct menu_item mi_no_ext[] = {
	{_(T_NO_EXTENSIONS), M_BAR, NULL, NULL, 0, 0},
	...
	foreachback (opt, *opt_tree) {
		...
		if (translated2 && optptr2) {
			add_to_menu(&mi, translated,
				    optptr2, MENU_FUNC fn, translated2, 0);
		} else {
