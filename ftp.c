...
int add_file_cmd_to_str(unsigned char **, int *, struct connection *);
...
void ftp_login(struct connection *c)
{
	...
	if (u) mem_free(u);
	add_to_str(&login, &logl, "\r\n");
	if (add_file_cmd_to_str(&login, &logl, c)) {
		mem_free(login);
		return;
	}
...
void ftp_got_user_info(struct connection *c, struct read_buffer *rb)
{
	...
	if (g >= 400) { setcstate(c, S_FTP_UNAVAIL); retry_connection(c); return; }
	if (g >= 200 && g < 300) ftp_dummy_info(c, rb);
	else ftp_pass_info(c, rb);
}
...
void ftp_pass_info(struct connection *c, struct read_buffer *rb)
{
	int g = get_ftp_response(c, rb, 0);
	if (g == -1) { setcstate(c, S_FTP_ERROR); abort_connection(c); return; }
	if (!g) { read_from_socket(c, c->sock1, rb, ftp_pass_info); return; }
...
int add_file_cmd_to_str(unsigned char **s, int *l, struct connection *c)
{
	unsigned char *d = get_url_data(c->url);
	unsigned char *de;
	unsigned char pc[6];
	int ps;
	struct ftp_connection_info *inf;
	...
	c->info = inf;
	if ((ps = get_pasv_socket(c, c->sock1, &c->sock2, pc))) return ps;
	if (!d) {
...
void ftp_send_retr_req(struct connection *c)
{
	unsigned char *login;
	int logl = 0;
	if (!(login = init_str())) {
