/* SSL socket workshop */
/* $Id: connect.c,v 1.69 2004/08/02 23:22:20 jonas Exp $ */
...
#define ssl_do_connect(conn)		SSL_get_error(conn->socket.ssl, SSL_connect(conn->socket.ssl))
...
#define ssl_do_connect(conn)		gnutls_handshake(*((ssl_t *) conn->socket.ssl))
