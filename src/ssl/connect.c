/* SSL socket workshop */
/* $Id: connect.c,v 1.102 2005/04/11 22:50:34 jonas Exp $ */
...
{
	struct conn_info *conn_info = conn->socket.conn_info;
	...
	switch (ret) {
		...
		case SSL_ERROR_WANT_READ2:
			...
			set_handlers(socket->fd, (void (*)(void *)) ssl_want_read,
				     NULL, dns_exception, socket->conn);
			return -1;
...
ssl_read(struct connection_socket *socket, struct read_buffer *rb)
{
	...
	if (rd <= 0) {
		...
		if (err == SSL_ERROR_WANT_READ ||
			...
			read_from_socket(rb->socket, rb, rb->done);
			return -1;
		}
