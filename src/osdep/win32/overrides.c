...
#ifdef CONFIG_MOUSE
static int console_mouse_read (const MOUSE_EVENT_RECORD *mer, char *buf, int max)
{
...
static int console_peek (HANDLE hnd)
{
	...
	while (num--)
	{
		...
		{
			WriteConsoleInput (hnd, &ir, 1, &written); /* put the record back */
			rc++;
		}
	}
	return (rc);
...
int win32_ioctl (int fd, long option, int *flag)
{
	...
	switch (what_fd_type(&fd))
	{
		...
		case FDT_PIPE:
			...
			break;
		case FDT_SOCKET:
			rc = ioctlsocket (fd, option, (u_long*)flag);
			if (rc < 0)
				errno = WSAGetLastError();
			break;
		default:
...
int win32_socket (int family, int type, int protocol)
{
...
static void select_dump (int num_fds,
		const fd_set *rd,
		const fd_set *wr,
		const fd_set *ex)
{
...
static int select_one_loop (int num_fds,
		struct fd_set *rd,
		struct fd_set *wr,
		struct fd_set *ex)
{
	...
	for (rc = fd = 0; fd < num_fds; fd++)
	{
		...
		{
			DWORD read = 0;
			if (PeekNamedPipe(hnd,NULL,0,NULL,&read,NULL) && read > 0)
			{
				FD_SET (fd, rd);
				rc++;
			}
