/* $Id: ruleset.h,v 1.1 2003/01/19 20:25:21 jonas Exp $ */
...
/* The setup:
 *
 * Rulesets with multiple selectors is dublicated. This will make the syntax
 ...
 * added to the underlaying selector states' css_node.
 *
...
/* Parsing of rulesets */
enum pstate_code
parse_ruleset(struct parser_state *state, unsigned char **src, int *len);

/* Parsing of selectors */
enum pstate_code
parse_selector(struct parser_state *state, unsigned char **src, int *len);

/* Parsing of a property declaration */
enum pstate_code
parse_declaration(struct parser_state *state, unsigned char **src, int *len);

/* Parsing of property settings */
enum pstate_code
parse_expression(struct parser_state *state, unsigned char **src, int *len);

/* Parsing of functions in selectors and expressions.
 * This can have optimized versions and just act as multiplexor. */
enum pstate_code
parse_function(struct parser_state *state, unsigned char **src, int *len);

/* Parsing of rgb functions */
enum pstate_code
parse_rgb(struct parser_state *state, unsigned char **src, int *len);
