...
void del_connection(struct connection *c)
{
	...
	mem_free(c->url);
	if (c->conn_info) {
		/* We must do it here, because when retrying connection,
		 * it probably may not be autogenerated! --pasky */
		if (*((void **) c->conn_info))
...
void connection_timeout(struct connection *c)
{
	...
	else if (c->conn_info) { dns_found(c, 0); if (c->conn_info) set_timeout(c); }
