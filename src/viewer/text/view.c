...
/* $Id: view.c,v 1.251 2003/10/31 01:59:40 jonas Exp $ */
...
draw_frames(struct session *ses)
{
	...
	*l %= n;
	...
	do {
		...
		foreach (doc_view, ses->scrn_frames) {
			if (doc_view->depth == d)
				draw_document_view(doc_view, ses->tab->term, doc_view == current_doc_view);
...
draw_formatted(struct session *ses)
{
	...
	draw_document_view(ses->doc_view, ses->tab->term, 1);
	draw_frames(ses);
	...
	if (ev->ev == EV_KBD) {
		...
		if ((ev->b & BM_BUTT) >= B_WHEEL_UP) {
			...
			if ((link->type == LINK_HYPERTEXT || link->type == LINK_BUTTON ||
				...
				draw_document_view(doc_view, ses->tab->term, 1);
				print_screen_status(ses);
				redraw_from_window(ses->tab);
